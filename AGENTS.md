...
`ShoppingList` + `ShoppingListItem` (migration `add_shopping_lists`, purely additive). A list is **group** (`groupId`) or **personal** (`ownerId`) — XOR discriminated; items are lightweight (name required, optional `quantity`/`unit`/`priceCents`/`note`) and carry **no `categoryId`** (the category is chosen once at checkout). All validation, scope/XOR enforcement, sortOrder allocation, the idempotent `checked` toggle (condition-by-id `updateMany`, never read-modify-write) and the bridge live in `src/lib/list-crud.ts` (reads in `list-read.ts`, error mapping in `list-http.ts`); routes own only authorization. **Bridge list→expense** (`checkoutList`): the checked+priced+unlinked items of a list materialize into ONE `Expense` reusing the real finance primitives (`resolveCategoryId` + `calculateSplitAmounts` + `postExpenseLedger`) — group lists → SHARED expense split equally + ledger, personal lists → PERSONAL expense (no split/ledger); idempotency is sealed by stamping `linkedExpenseId` in the same transaction (guarded on `linkedExpenseId:null`), so a retry never double-posts. Routes: space `/api/spaces/[id]/lists/**` (authorized via `requireSpaceAccess`, `allowGuest:false`, writability blocks SETTLING/ARCHIVED) and personal `/api/me/lists/**` (session-scoped by `ownerId`), each with `…/[listId]/checkout`. UI under `/lists` (índice Común/Personal + detalle) — `/lists` is proxy-protected.
