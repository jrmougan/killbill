...
**OCR reconciliation** (`…/[listId]/reconcile`, `receipt-reconcile.ts` + `reconcile-handler.ts`): a receipt photo is matched against the list's **pending (unchecked)** items via a dedicated Gemini vision call (fuzzy match — receipts are cryptic uppercase abbreviations vs. generic list names). It is **additive and confirm-only**: the endpoint returns SUGGESTED matches (item id + matched receipt text + clamped confidence), it **never marks anything** — the user confirms and each tick goes through the normal idempotent `setItemChecked` toggle. It needs `GEMINI_API_KEY`; without it the reconcile endpoint degrades cleanly (500 with a clear error) and the base receipt OCR is unaffected. Rate-limited (10 vision calls / 5 min per user).

Routes: space `/api/spaces/[id]/lists/**` (authorized via `requireSpaceAccess`, `allowGuest:false`, writability blocks SETTLING/ARCHIVED) and personal `/api/me/lists/**` (session-scoped by `ownerId`), each with `…/[listId]/clear-checked` (deletes the checked items to recycle the weekly list) and `…/[listId]/reconcile`. `reconcile`/`clear-checked`/item routes all re-assert the list belongs to the scope (`getListWithItems`/`loadListInScope`) after the space-access gate. UI under `/lists` (índice Común/Personal + detalle) — `/lists` is proxy-protected.
