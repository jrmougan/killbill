...
interface ImportRow {
    ...
    description: string;
    category?: string;    // category key; defaults to 'other'
}
...
export async function POST(request: Request) {
    ...
    // Resolve category ids once (personal expenses resolve to the system category).
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        data.push({
            ...
            categoryId: await categoryIdFor(r.category ?? 'other'),
            paidById: userId,
            ownerId: userId,
            visibility: 'PERSONAL',
            coupleId: null,
